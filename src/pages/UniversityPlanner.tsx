...
const UNIVERSITY_DRAFT_KEY = "planner:university:draft";
...
export default function UniversityPlanner() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem(UNIVERSITY_DRAFT_KEY);
      if (!stored) {
        return;
      }
      const draft = JSON.parse(stored) as Partial<{
        ...
        minor: string;
        programs: string;
        yearsToFinish: string | number;
        semesters: SemesterData[];
      }>;
      setPlanName(draft.planName ?? "My Academic Plan");
      setUniversity(draft.university ?? "");
      setMajor(draft.major ?? "");
      setMinor(draft.minor ?? "");
      setPrograms(draft.programs ?? "");
      const yearsValue = draft.yearsToFinish ? String(draft.yearsToFinish) : "4";
      setYearsToFinish(yearsValue);
      if (draft.semesters && draft.semesters.length > 0) {
        setSemesters(draft.semesters);
      } else {
        setSemesters(generateUniversitySemesters(parseInt(yearsValue, 10)));
      }
    } catch {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(UNIVERSITY_DRAFT_KEY, JSON.stringify(draft));
  }, [planName, university, major, minor, programs, yearsToFinish, semesters]);
      ...
      const response = await fetch(apiUrl('/api/generate-plan'), {
        ...
        body: JSON.stringify({
          ...
          inputs: {
            ...
            minor,
            programs,
            yearsToFinish: parseInt(yearsToFinish),
          },
  ...
  const handleClearPlan = () => {
    ...
    setPrograms("");
    setYearsToFinish("4");
    setSemesters(generateUniversitySemesters(4));
    if (typeof window !== "undefined") {
      localStorage.removeItem(UNIVERSITY_DRAFT_KEY);
    }
    toast({
      title: "Plan Cleared",
